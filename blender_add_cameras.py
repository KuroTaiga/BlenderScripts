...
if armature is None:
    ...
    if spine2_bone is None:
        ...
        # # Rotate cameras every 30 degrees, for a total of 360 degrees
        # rotation_step_degree = 30
        # rotation_steps = 12
        # we want to keep the following camera angles faceing the person: 
        # right (0) front (270) back (90) left-behind (150) left/right-front (240,300)
        rotation_steps = 6
        rotation_degrees = [0, 90, 150, 240, 270, 300] 
        ...
        for z_pos in z_positions:
            for radius in radii:
                for step in range(rotation_steps):
                    ...
                    angle = math.radians(rotation_degrees[step])
                    ...
                    camera.name = f"{blend_name}_Camera_R{int(radius*100)}_Z{int(z_pos*100)}_A{rotation_degrees[step]}"
                    camera_counter += 1
